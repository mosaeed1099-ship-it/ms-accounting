...
# جلب واحد فقط — compressed — يُستخدم لكل الفحوصات
TIME=$(curl -s -o /dev/null -w "%{time_total}" --max-time 15 --compressed "$SITE/")
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 15 --compressed "$SITE/")
curl -s --compressed --max-time 30 -o /tmp/_post_html.html "$SITE/" 2>/dev/null
SIZE=$(wc -c < /tmp/_post_html.html | tr -d ' ')
...
# ─── 2. قراءة الـ HTML من الملف المحفوظ ─────────────────────
HTML=$(cat /tmp/_post_html.html)
