    ...
    return `<div style="background:white;border:1px solid #b0b0b0;border-radius:6px;margin-bottom:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08)">
      ...
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:12px;min-width:540px">
          ...
          <tbody id="ptbody_${u.id}">
            ${pendingT.length===0?`<tr id="empty_${u.id}"><td colspan="9" style="text-align:center;padding:12px;font-size:11px;color:#15803d;font-weight:600;border:${XL_BORDER}">🎉 جميع المهام مكتملة</td></tr>`:''}
            ${rows}
          </tbody>
        </table>
      </div>
      ...
      <div style="padding:8px 12px;background:var(--color-background-secondary);border-top:0.5px dashed var(--color-border-tertiary);display:flex;align-items:center;gap:8px">
        <input id="newt_${u.id}" class="input" placeholder="اكتب مهمة جديدة واضغط Enter..."
          ...
          onkeydown="if(event.key==='Enter'){event.preventDefault();window._addEmpTask(${u.id});}"/>
...
window._addEmpTask = async function(uid) {
  ...
  inp.value='';
  inp.focus(); // الـ focus لا يتفقد أبداً — مفيش renderDailySheet هنا

  const tempId='tmp_'+Date.now();
  const tempTask={id:tempId,title:txt,assigned_to:uid,status:'todo',priority:'medium',category:'other',
    created_at:new Date().toISOString(),due_date:null,carry_over_days:0,client_name:null,_optimistic:true};
  tasksData.push(tempTask);

  // حقن الـ row مباشرةً في الـ DOM بدون rebuild — الـ input يفضل شغال
  const tbody=document.getElementById('ptbody_'+uid);
  if(tbody){
    document.getElementById('empty_'+uid)?.remove();
    const rowN=tbody.querySelectorAll('tr').length+1;
    const B='1px solid #c6c6c6';
    const xl=(e='')=>`border:${B};padding:5px 8px;${e}`;
    const tr=document.createElement('tr');
    tr.id='optrow_'+tempId;
    tr.style.cssText='background:#eef6ff;opacity:.75';
    tr.innerHTML=
      `<td style="${xl('text-align:center;font-size:10px;color:#555;width:3%')}">${rowN}</td>`+
      `<td style="${xl('width:36%')}"><span style="font-size:11.5px;color:#1a2472">⏳ ${escH(txt)}</span></td>`+
      `<td style="${xl('text-align:center;font-size:10px;color:#aaa;width:8%')}">—</td>`+
      `<td style="${xl('text-align:center;font-size:10px;color:#aaa;width:9%')}">—</td>`+
      `<td style="${xl('text-align:center;width:12%')}"></td>`+
      `<td style="${xl('text-align:center;width:6%')}"></td>`+
      `<td style="${xl('text-align:center;width:8%')}"></td>`+
      `<td style="${xl('text-align:center;width:6%')}"></td>`+
      `<td style="${xl('text-align:center;width:6%')}"></td>`;
  ...
  try {
    ...
    // حدّث الـ row الـ optimistic بالـ id الحقيقي والأزرار
    const tr=document.getElementById('optrow_'+tempId);
    if(tr){
      tr.id='taskrow_'+res.id;
      tr.style.background='';
      tr.style.opacity='1';
      const tds=tr.querySelectorAll('td');
      const B2='1px solid #c6c6c6';
      const xl2=(e='')=>`border:${B2};padding:5px 8px;${e}`;
      if(tds[1]) tds[1].innerHTML=`<span style="color:#1a1a1a;font-size:11.5px">${escH(txt)}</span>`;
      if(tds[5]) tds[5].innerHTML=`<div onclick="window._markDone(${res.id})" style="width:20px;height:20px;border-radius:3px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;transition:all .12s;background:white;border:1.5px solid #999;color:transparent">✓</div>`;
      if(tds[7]) tds[7].style.cssText=xl2('text-align:center;width:6%'),tds[7].innerHTML=`<button onclick="showTaskModal(${res.id})" style="background:#e8f0fe;border:1px solid #aac4ff;border-radius:4px;padding:2px 7px;cursor:pointer;font-size:11px;color:#1a2472">✏️</button>`;
      if(tds[8]) tds[8].style.cssText=xl2('text-align:center;width:6%'),tds[8].innerHTML=`<button onclick="deleteTask(${res.id},null)" style="background:#fce8e6;border:1px solid #f5c6c2;border-radius:4px;padding:2px 7px;cursor:pointer;font-size:11px;color:#c00">🗑️</button>`;
    }
  } catch(e){
    // لا تمسح المهمة — خليها تفضل مع زرار إعادة المحاولة
    const trFail=document.getElementById('optrow_'+tempId);
    if(trFail){
      trFail.style.background='#fff5f5';
      trFail.style.opacity='1';
      const td=trFail.querySelector('td:nth-child(2)');
      if(td) td.innerHTML=`<span style="color:#dc2626;font-size:11px">⚠️ ${escH(txt)}</span>
        <button onclick="window._retryTask('${tempId}',${uid},this)" style="margin-right:8px;background:#dc2626;color:white;border:none;border-radius:4px;padding:2px 10px;font-size:10px;cursor:pointer;font-family:inherit">↻ إعادة المحاولة</button>`;
    }
    toast('فشل حفظ المهمة — '+e.message,'error');
  }
};

window._retryTask = async function(tempId, uid, btn) {
  const tr=document.getElementById('optrow_'+tempId);
  if(!tr) return;
  const td=tr.querySelector('td:nth-child(2)');
  const txt=td?.querySelector('span')?.textContent?.replace(/^⚠️\s*/,'');
  if(!txt) return;
  if(btn){btn.disabled=true;btn.textContent='⏳';}
  try {
    const res=await api('POST','/api/tasks',{title:txt,assigned_to:uid,status:'todo',priority:'medium',category:'other'},{useCache:false});
    const idx=tasksData.findIndex(t=>t.id===tempId);
    if(idx>=0) tasksData[idx]={...tasksData[idx],...res,id:res.id,_optimistic:false};
    _AC.invalidate('/api/tasks');
    tr.id='taskrow_'+res.id;
    tr.style.background='';
    const B='1px solid #c6c6c6';
    const xl=(e='')=>`border:${B};padding:5px 8px;${e}`;
    if(td) td.innerHTML=`<span style="color:#1a1a1a;font-size:11.5px">${escH(txt)}</span>`;
    const tds=tr.querySelectorAll('td');
    if(tds[5]) tds[5].innerHTML=`<div onclick="window._markDone(${res.id})" style="width:20px;height:20px;border-radius:3px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;transition:all .12s;background:white;border:1.5px solid #999;color:transparent">✓</div>`;
    if(tds[7]) tds[7].innerHTML=`<button onclick="showTaskModal(${res.id})" style="background:#e8f0fe;border:1px solid #aac4ff;border-radius:4px;padding:2px 7px;cursor:pointer;font-size:11px;color:#1a2472">✏️</button>`;
    if(tds[8]) tds[8].innerHTML=`<button onclick="deleteTask(${res.id},null)" style="background:#fce8e6;border:1px solid #f5c6c2;border-radius:4px;padding:2px 7px;cursor:pointer;font-size:11px;color:#c00">🗑️</button>`;
    toast('✅ تم حفظ المهمة');
  } catch(e){
    if(btn){btn.disabled=false;btn.textContent='↻ إعادة المحاولة';}
    toast('فشل مجدداً — '+e.message,'error');
  }
};
...
async function deleteTask(id, overlay) {
  ...
  overlay?.remove();
  // Optimistic: أزل من الواجهة فوراً
  tasksData=tasksData.filter(t=>t.id!==id);
  renderDailySheet();
  toast('تم حذف المهمة');
  // ثم احذف من السيرفر في الخلفية
  api('DELETE',`/api/tasks/${id}`,null,{useCache:false}).then(()=>{
    _AC.invalidate('/api/tasks');
