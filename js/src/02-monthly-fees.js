...
let mfCurrentYear  = new Date().getFullYear();
let mfCurrentMonth = new Date().getMonth() + 1; // يتحدث تلقائياً مع بداية كل شهر
...
async function renderMFPage() {
  ...
  if (Array.isArray(records) && records.length === 0 && Array.isArray(mfClients) && mfClients.length > 0) {
    ...
    _mfAllRecords = newRecords;
    window._mfLastDash = newDash;
    return mfRenderPage(newDash, newRecords);
  }
