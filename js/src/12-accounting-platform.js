...
async function loadAccountingPlatform() {
  ...
  if (!main) return;
  main.className = 'page';
  main.innerHTML = '<div style="padding:28px;text-align:center;color:#6b7280;font-size:.95rem">⏳ جاري تحميل مركز الخدمات...</div>';

  let clients = [];
  try {
    const raw = await api('GET', '/api/clients');
    clients = !raw ? [] : Array.isArray(raw) ? raw : (raw.items || []);
  } catch(e) { clients = []; }

  if (!clients.length) {
    main.innerHTML = '<div style="padding:36px;text-align:center;color:#6b7280">لا يوجد عملاء مسجلون</div>';
    return;
  }
