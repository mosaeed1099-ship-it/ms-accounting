...
async function accTransactions(type) {
  ...
  let batchesHtml = '';
  try {
    const batchData = await api('GET', `/api/accounting/${_accClientId}/import/batches?section_type=${type}`);
    const batches = batchData.batches || [];
    if(batches.length > 0) {
      batchesHtml = `
      <details style="margin-bottom:12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px">
        <summary style="padding:10px 14px;cursor:pointer;font-size:12px;font-weight:700;color:#475569">
          📂 سجل استيرادات ${SECTION_AR[type]||type} (${batches.length})
        </summary>
        <div style="padding:8px 14px 12px">
          ${batches.map(b=>`
          <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #e8edf3;font-size:11px;gap:8px">
            <div>
              <div style="font-weight:600;color:#1e293b">📄 ${escH(b.filename)}</div>
              <div style="color:#64748b">${(b.imported_at||'').substring(0,10)} — ${b.tx_count} معاملة — ${b.je_count} قيد</div>
            </div>
            <button onclick="deleteImportBatch(${b.id})" style="background:#fee2e2;border:none;border-radius:6px;padding:3px 8px;cursor:pointer;color:#dc2626;font-size:11px;white-space:nowrap">🗑 حذف</button>
          </div>`).join('')}
        </div>
      </details>`;
    }
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:8px">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      <label style="background:#f0f9ff;border:1.5px solid #bae6fd;border-radius:8px;padding:6px 12px;font-size:13px;cursor:pointer;color:#0369a1;font-weight:600;display:flex;align-items:center;gap:5px">
        📥 استيراد
        <input type="file" accept=".xlsx,.xls" style="display:none" onchange="accImportExcelForType('${type}', this.files[0])">
      </label>
...
// ── Per-Section Excel Import ───────────────────────────────────────────────────
const SECTION_AR = {sale:'المبيعات', purchase:'المشتريات', expense:'المصروفات', asset:'الأصول الثابتة', salary:'المرتبات'};

async function accImportExcelForType(type, file) {
  if(!file) return;
  window._importFilename = file.name;
  window._importSection  = type;
  const formData = new FormData();
  formData.append('file', file);
  formData.append('section', type);
  try {
    toast(`⏳ جاري تحليل ملف ${SECTION_AR[type]||type}...`, 'info');
    const r = await fetch(`${API}/api/accounting/${_accClientId}/import/excel/preview`, {
      method: 'POST',
      headers: token ? {Authorization: `Bearer ${token}`} : {},
      body: formData,
    ...
    const data = await r.json();
    if(!r.ok) throw new Error(data.detail || 'فشل التحليل');
    if(data.needs_sheet_selection && (data.sheet_candidates||[]).length > 1) {
      showSheetPicker(data.sheet_candidates, data);
    } else {
      showImportPreview(data);
    }
  } catch(e) { toast(e.message, 'error'); }
...
function showSheetPicker(candidates, fullData) {
  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay';
  overlay.style.zIndex = '1400';
  overlay.innerHTML = `<div class="modal" style="max-width:500px">
    <div style="padding:16px 22px;background:linear-gradient(135deg,#1a2472,#152060);border-radius:18px 18px 0 0;color:white;font-size:14px;font-weight:700">
      📋 وُجد أكثر من شيت صالح — اختر الشيت الذي تريد استيراده
    </div>
    <div style="padding:16px 22px">
      ${candidates.map(c=>`
        <div onclick="pickSheet('${escH(c.sheet)}', this)" style="border:2px solid #e8edf3;border-radius:10px;padding:12px 16px;margin-bottom:10px;cursor:pointer;transition:all .15s" onmouseover="this.style.borderColor='#1a2472'" onmouseout="if(!this.classList.contains('picked')) this.style.borderColor='#e8edf3'">
          <div style="font-weight:700;color:#1e293b;margin-bottom:4px">📄 ${escH(c.sheet)}</div>
          <div style="display:flex;gap:12px;font-size:12px;color:#64748b">
            <span>ثقة: <strong>${c.confidence}%</strong></span>
            <span>صفوف: <strong>${c.row_count}</strong></span>
            <span>نوع: <strong>${TX_TYPE_LABELS[c.tx_type]||c.tx_type}</strong></span>
          </div>
        </div>`).join('')}
    </div>
    <div style="padding:12px 22px;border-top:1px solid #e8edf3;text-align:left">
      <button class="btn btn-secondary" onclick="this.closest('.modal-overlay').remove()">إلغاء</button>
    </div>
  </div>`;
  document.body.append(overlay);
  ...
  window._pickerFullData = fullData;
  window.pickSheet = function(sheetName, el) {
    overlay.remove();
    // Filter data to only the selected sheet
    const filtered = {...window._pickerFullData};
    filtered.rows = (filtered.rows||[]).filter(r=>r.sheet===sheetName);
    filtered.error_rows = (filtered.error_rows||[]).filter(r=>r.sheet===sheetName);
    filtered.sheets = (filtered.sheets||[]).filter(s=>s.sheet===sheetName || s.skipped);
    filtered.new_rows_count   = filtered.rows.length;
    filtered.ok_rows_count    = filtered.rows.filter(r=>r.status==='ok').length;
    filtered.warning_rows_count = filtered.rows.filter(r=>r.status==='warning').length;
    filtered.error_rows_count = filtered.error_rows.length;
    showImportPreview(filtered);
  };
}
...
function showImportPreview(data) {
  ...
  window._importErrorRows = data.error_rows || [];
  ...
  const statusBadge = (s) => {
    if(s==='ok')      return '<span style="background:#dcfce7;color:#15803d;padding:1px 7px;border-radius:5px;font-size:10px;font-weight:700">✓ سليم</span>';
    if(s==='warning') return '<span style="background:#fef9c3;color:#92400e;padding:1px 7px;border-radius:5px;font-size:10px;font-weight:700">⚠ تحذير</span>';
    return              '<span style="background:#fef2f2;color:#dc2626;padding:1px 7px;border-radius:5px;font-size:10px;font-weight:700">✗ خطأ</span>';
  };
  const rowBg = (s) => s==='ok'?'':'s'==='warning'?'background:#fffbeb':'background:#fff5f5';

  const sheetsHtml = (data.sheets || []).map(s => {
    if(s.skipped) return `
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:10px 14px;margin-bottom:8px;opacity:0.7">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div style="font-size:12px;color:#64748b">⏭️ تم تخطي: <strong>${escH(s.sheet)}</strong></div>
          <div style="font-size:10px;color:#94a3b8">${escH(s.skip_reason||'')}</div>
        </div>
      </div>`;
    if(s.error) return `
    ...
    const errCount   = s.error_rows_count || 0;
    const warnCount  = s.warning_rows_count || 0;
    const okCount    = s.ok_rows_count || 0;
    ...
    const overrideWarning = s.section_type_overridden
      ? `<div style="background:#eff6ff;border:1px solid #93c5fd;border-radius:7px;padding:7px 12px;margin-bottom:8px;font-size:11px;color:#1e40af">
           ℹ️ <strong>تجاوز نوع مكتشف</strong> — النظام اكتشف هذا الشيت كنوع مختلف لكن القسم المحدد هو المصدر الحقيقي، تم الاستيراد حسب القسم.
         </div>` : '';
    const colMapHtml = `
      ${overrideWarning}${mappingWarning}
    ...
    // مقارنة المجاميع + diff_analysis
    let totalsHtml = '';
    if(s.sheet_total_amount > 0) {
      let diffDetail = '';
      if(!totalsOk && s.diff_analysis) {
        const da = s.diff_analysis;
        diffDetail = `<div style="margin-top:6px;border-top:1px solid #fecaca;padding-top:6px">
          <div style="font-weight:700;font-size:11px;color:#dc2626;margin-bottom:3px">سبب الفرق:</div>
          ${(da.reasons||[]).map(r=>`<div style="font-size:11px;color:#7f1d1d;padding-right:8px">• ${escH(r)}</div>`).join('')}
          ${da.error_rows_count > 0 ? `<div style="font-size:10px;color:#64748b;margin-top:3px">${da.error_rows_count} صف تم تجاهله بسبب أخطاء — يمكن استيراد الصفوف السليمة فقط</div>` : ''}
        </div>`;
      }
      totalsHtml = `
    ...
    const statsHtml = `
      <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
        <span style="background:#dcfce7;color:#15803d;border-radius:6px;padding:2px 10px;font-size:11px;font-weight:700">✓ ${okCount} سليم</span>
        ${warnCount > 0 ? `<span style="background:#fef9c3;color:#92400e;border-radius:6px;padding:2px 10px;font-size:11px;font-weight:700">⚠ ${warnCount} تحذير</span>` : ''}
        ${errCount  > 0 ? `<span style="background:#fef2f2;color:#dc2626;border-radius:6px;padding:2px 10px;font-size:11px;font-weight:700">✗ ${errCount} خطأ</span>` : ''}
      </div>`;

    // جدول الصفوف الخاطئة
    let errorRowsHtml = '';
    if((s.error_rows||[]).length > 0) {
      errorRowsHtml = `
        <div style="margin-bottom:8px">
          <div style="font-size:11px;font-weight:700;color:#dc2626;margin-bottom:4px">الصفوف بأخطاء (${s.error_rows.length}):</div>
          <div style="overflow-x:auto">
            <table style="width:100%;border-collapse:collapse;font-size:10px">
              <thead><tr style="background:#fef2f2">
                <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #fecaca">صف</th>
                <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #fecaca">الحقل</th>
                <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #fecaca">قيمة الشيت</th>
                <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #fecaca">السبب</th>
              </tr></thead>
              <tbody>
                ${s.error_rows.map(er => (er.errors||[]).map(e => `
                  <tr style="border-bottom:1px solid #fff5f5;background:#fff8f8">
                    <td style="padding:3px 7px;font-weight:700;color:#dc2626">${er.row_num}</td>
                    <td style="padding:3px 7px;color:#7c3aed">${escH(e.field)}</td>
                    <td style="padding:3px 7px;color:#374151;direction:ltr">${escH(e.sheet_value != null ? String(e.sheet_value) : '—')}</td>
                    <td style="padding:3px 7px;color:#dc2626">${escH(e.reason)}</td>
                  </tr>`).join('')).join('')}
              </tbody>
            </table>
          </div>
    ...
    // جدول الصفوف السليمة (sample)
    const sampleHtml = (s.sample||[]).length ? `
      <details style="margin-top:4px">
        <summary style="font-size:11px;font-weight:700;color:#475569;cursor:pointer;margin-bottom:4px">عرض عينة الصفوف السليمة (${s.ok_rows_count + s.warning_rows_count})</summary>
        <div style="overflow-x:auto;margin-top:6px">
          <table style="width:100%;border-collapse:collapse;font-size:10px">
            <thead><tr style="background:#f8fafc">
              <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #e8edf3">التاريخ</th>
              <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #e8edf3">الجهة</th>
              <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #e8edf3">مستند</th>
              <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #e8edf3">المبلغ</th>
              <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #e8edf3">ض.ق.م</th>
              <th style="padding:3px 7px;text-align:right;color:#64748b;border-bottom:1px solid #e8edf3">حالة</th>
            </tr></thead>
            <tbody>
              ${s.sample.map(r => {
                const bg = r.status==='warning' ? ';background:#fffbeb' : '';
                return `<tr style="border-bottom:1px solid #f1f5f9${bg}">
                  <td style="padding:3px 7px">${r.date||'—'}</td>
                  <td style="padding:3px 7px">${escH(r.partner||r.description||'—')}</td>
                  <td style="padding:3px 7px;color:#64748b">${escH(r.doc_number||'—')}</td>
                  <td style="padding:3px 7px;font-weight:700">${fmt(r.amount)}</td>
                  <td style="padding:3px 7px;color:#7c3aed">${fmt(r.vat)}</td>
                  <td style="padding:3px 7px">${statusBadge(r.status)}</td>
                </tr>`;
              }).join('')}
            </tbody>
          </table>
    ...
    return `
    <div style="background:white;border:1px solid ${borderCol};border-radius:10px;padding:12px 14px;margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          ...
          <span style="font-size:11px;color:#64748b">${s.raw_rows_count||0} صف إجمالاً</span>
        </div>
      </div>
      ${colMapHtml}
      ${statsHtml}
...
function downloadImportErrorReport() {
  const errorRows = window._importErrorRows || [];
  if(!errorRows.length) { toast('لا توجد أخطاء لتحميلها', 'info'); return; }

  const lines = [['رقم الصف','الشيت','رقم المستند','التاريخ (الشيت)','المبلغ (الشيت)','الحقل','السبب'].join(',')];
  errorRows.forEach(er => {
    (er.errors||[]).forEach(e => {
      lines.push([
        er.row_num,
        `"${(er.sheet||'').replace(/"/g,'""')}"`,
        `"${(er.doc_number||'').replace(/"/g,'""')}"`,
        `"${(er.raw_date||'').replace(/"/g,'""')}"`,
        `"${(er.raw_amount||'').replace(/"/g,'""')}"`,
        `"${(e.field||'').replace(/"/g,'""')}"`,
        `"${(e.reason||'').replace(/"/g,'""')}"`,
      ].join(','));
    });
...
async function confirmImportExcel(goodOnly = false) {
  const allRows  = window._importRows || [];
  const rows = goodOnly ? allRows.filter(r => r.status !== 'error') : allRows;
  if(!rows.length) { toast('لا توجد صفوف للاستيراد', 'info'); return; }
  const btn = document.getElementById(goodOnly ? 'confirmGoodOnlyBtn' : 'confirmImportBtn');
  if(btn) { btn.disabled = true; btn.textContent = '⏳ جاري الترحيل...'; }
  try {
    const filename = window._importFilename || 'ملف Excel';
    const r = await api('POST', `/api/accounting/${_accClientId}/import/excel/confirm`, { rows, filename, import_good_only: goodOnly, section_type: window._importSection || null });
    ...
    toast(e.message, 'error');
    if(btn) { btn.disabled = false; btn.textContent = btn.id === 'confirmGoodOnlyBtn' ? `✅ استيراد السليم فقط (${rows.length})` : `✅ اعتماد وترحيل ${rows.length} معاملة`; }
  }
}
...
async function deleteImportBatch(batchId, filename, txCount) {
  const label = filename ? `"${filename}" (${txCount} معاملة وجميع قيودها)` : `Batch #${batchId}`;
  if(!await confirmDlg(`حذف عملية الاستيراد ${label}؟\n\nلن يتأثر أي استيراد آخر.`)) return;
  try {
...
async function renderEmpSettlements() {
  ...
  try {
    ...
    const now = new Date();
    const isCurrentMonth = (_callMonth === now.getMonth()+1 && _callYear === now.getFullYear());
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:18px">
      <div style="display:flex;align-items:center;gap:12px">
        ...
        <div>
          ...
          <p style="font-size:12px;color:#64748b;margin:2px 0 0">تسويات ${monthNames[_callMonth-1]} ${_callYear}</p>
        </div>
      </div>
      <div style="display:flex;gap:8px;align-items:center">
    ...
    <div style="background:white;border-radius:14px;border:1.5px solid #e8edf3;overflow:hidden">
      <div style="padding:14px 18px;border-bottom:1px solid #f1f5f9;display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:14px;font-weight:700;color:#1e293b">📋 سجل التسويات — ${monthNames[_callMonth-1]} ${_callYear}</span>
...
window.accImportExcelForType = accImportExcelForType;
window.showSheetPicker = showSheetPicker;
