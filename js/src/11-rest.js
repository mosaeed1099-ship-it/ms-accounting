...
function addStlItem(data={}) {
  ...
  const catOpts = STL_CATS.map(c=>`<option${(data.description||data.cat||'')==c?' selected':''}>${c}</option>`).join('');
  row.innerHTML = `
    ...
    <div style="${cellStyle}"><select data-f="cat" style="width:100%;padding:5px 6px;border:none;font-size:12px;font-family:inherit;background:transparent" onchange="calcStlTotal()">
      <option value="">اختر البند</option>${catOpts}
      <option${!STL_CATS.includes(data.description||'')&&(data.description)?' selected':''} value="${escH(data.description||'')}">
        ${STL_CATS.includes(data.description||'')?'':escH(data.description||'')}
      </option>
    </select></div>
